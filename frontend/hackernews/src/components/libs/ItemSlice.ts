...
export const updateItem = createAsyncThunk(
  ...
  async (updateItem: Item) => {
    const response = await axiostInstance.put(
      getExtraParamsEndPoint(updateItem.id, "update")
    );
    return response.data;
  }
);
...
export const deleteItem = createAsyncThunk(
  "items/deleteItem",
  async (id: number) => {
    await axiostInstance.delete(getExtraParamsEndPoint(id, 'delete'));
    return id;
  }
);
